...
PlaylistComponent::PlaylistComponent(Deck *_deckOne, Deck *_deckTwo) : deckOne(_deckOne), deckTwo(_deckTwo)
{
    ...
    tableComponent.getHeader().addColumn("Deck One", 3, 200);
    tableComponent.getHeader().addColumn("Deck Tow", 4, 200);
...
void PlaylistComponent::filesDropped(const StringArray &files, int x, int y)
{
    ...
    {
        std::unique_ptr<AudioFormatReader> reader(formatManager.createReaderFor(File{file}));
//        ScopedPointer<AudioFormatReader> reader = formatManager.createReaderFor (File{file});

        OutputProcessor::info("Loaded " + std::to_string(reader->metadataValues.size()));
        
        for (String key : reader->metadataValues.getAllKeys()) {
               DBG ("Key: " + key + " value: " + reader->metadataValues.getValue (key, "unknown"));
           }
        
//        for( StringArray str : reader->metadataValues.getAllValues() ) {
//
//            OutputProcessor::info(str.getReference(0).toStdString());
//
//        }
...
String PlaylistComponent::getLengthOfTrack(String file)
{
    ...
    {
//        OutputProcessor::info(std::to_string(reader->metadataValues[0]));
